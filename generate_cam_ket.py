...
new_content = """<div class="print-page">
...
# Replace content div
pattern = r'<div id="content">.*?</div>\s*<script>'
replacement = f'<div id="content">\n{new_content}\n    </div>\n    <script>'
html = re.sub(pattern, replacement, html, flags=re.DOTALL)

# Add logic for NgaySinh and Dia_Chi_Tram
script_additions = """
                        setTextForAll('Dia_Chi_Tram', data.DiaChiTram);
                        if (data.NgaySinh) {
                            const dateSinh = new Date(data.NgaySinh);
                            if (!isNaN(dateSinh.getTime())) {
                                setTextForAll('Ben_A_Ngay_Sinh', dateSinh.toLocaleDateString('vi-VN'));
                            }
                        }
"""
html = html.replace("setTextForAll('Dien_Tich_Thue', data.DienTichThue_m2);", "setTextForAll('Dien_Tich_Thue', data.DienTichThue_m2);" + script_additions)

with open('d:/OneDrive - VNPT/AI/thueCSHT/vercel-static-hopdong/cam_ket.html', 'w', encoding='utf-8') as f:
    f.write(html)
